	...
	- [Carion et al_2020_End-to-End Object Detection with Transformers.pdf](zotero://open-pdf/library/items/LJKH5VA3)
		- [[abstract]]:
			...
			- Our approach streamlines the detection pipeline, effectively removing the need for many hand-designed components that explicitly encode our prior knowledge about the task.
				- 取代现在模型需要手工设计的工作 No geometric priors
					- [[NMS]]
					- anchor generation
...
- 1. DETR Architecture
  heading:: true
	...
	- Preprocess中变量的变化过程
		- ![image.png](../assets/pages_detr_1615535873210_0.png){:height 540, :width 727}
		-
	- 1.1 Backbone
	  heading:: true
	...
	- 1.2 encoder
	  heading:: true
		- 主要使用了 [[self-attention]] 提取不同之间的interactions
	...
	- 1.3 Decoder
	  heading:: true
		- 跟标准transformer区别在于
			...
			- apply **supervision** after each layer
				- dynamically choose how many layers to choose from a trained model
		- Decoder两个输入
			- Encoder输出的Embedding与position encoding之和
			- Object queries
		- 因为transformer decoder结构是**permutation-invariant**的
			- $N$个input embeddings must be different to produce different results
				- shape $(100, 256)$
				- 被称为^^object queries^^ (input of the decoder layers)
					- Query spatial specialization
						- Average box center predicted by each query over the [[COCO]] validation set
	...
	- ![image.png](../assets/pages_detr_1613987449992_0.png){:height 522, :width 482}
	- ![image.png](../assets/pages_detr_1615539393940_0.png){:height 601, :width 680}
- 2. Loss
  heading:: true
	  ...
	  heading:: true
		...
		- (2) 计算loss function
			...
			- Bounding box loss $\mathcal{L}_{box}$
				- make box predictions _directly_
					- 不同与其他detectors w.r.t. initial guesses - offset
				- 为了解决scale的问题
				...
				- $\mathcal{L}_{box}(b_i, \hat{b}_{\sigma(i)})$ defined as:
					...
					- normalized by the no. of objects inside the batch
	-